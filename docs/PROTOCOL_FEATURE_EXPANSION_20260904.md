...
**日期**：2026-09-04
**性质**：表示容量扩充 + 诊断。**两个嵌套配置，共同主判据（co-primary），互不替代。只测不选** —— 不让流程在新池上做任何删除决策。
...
| `full94+N33` | 127 | 13 | `full94` 左连接 `N33`（§2.4）—— **共同主判据 A** |
| `full94+N89` | **183** | **15** | `full94` 左连接全部 89 新列 —— **共同主判据 B** |
...
**共同主判据（co-primary），两个，运行前定死，互不替代**：

| | 配置 | 性质 |
|---|---|---|
| **主判据 A** | `full94+N33` | **预注册的证据靶单** —— 于 2026-09-04 先冻结（sha256 `56ed0b20…`），**在 89 列存在之前、在任何数据之前**，据 `UNSW-FAMILY-REVERSAL` / `UNSW-BELKIN-CAPACITY` / 自采 `diag_coord1.csv` 三处已测结果推导。**故不是事后从 89 中挑的子集。** |
| **主判据 B** | `full94+N89` | 未经筛选的超集，用于检验 A 的筛选是否遗漏 |

`Δceiling_A = joint_tb(full94+N33) − joint_tb(full94)`；
`Δceiling_B = joint_tb(full94+N89) − joint_tb(full94)`。
`joint_tb` 沿用 `run_histint.joint_eval`（H = R2+R3+R4，前 4 时间块训练、第 5 块测试），
取 `best_base`（三非 stacking 模型逐种子取 max），五种子均值。
**`joint_tb(full94)` 在本协议内重跑取得**（同 runner、同估计量）；外部参照 0.9084
（`r567_94dim_retest_20260904/deltas.csv`，sd 0.0011），若相差 > 0.005 须在偏离段说明。

**每个主判据各自独立落一个分支，判定为一个「对」**（如 `(RAISES, MARGINAL)`）：

| 分支 | 条件 | 处置 |
|---|---|---|
| `EXPANSION_RAISES_CEILING` | `Δceiling ≥ +0.02` | 该配置含现有 94 列没有的信息 → 移植到 UNSW（另立协议）测 Belkin |
| `EXPANSION_MARGINAL` | `0 ≤ Δceiling < +0.02` | 噪声量级 → 如实报，不移植 |
| `EXPANSION_USELESS` | `Δceiling < 0` | 该配置纯噪声（更多列反而稀释） |

**三条不得违反的规定**：

1. **A 与 B 互不替代。**任何一个的数值都不得代替另一个作为「本协议的结论」；
   **两个判定必须同时报出**，使读者能看见「只有一个过」这种情形。
2. **两个共同主判据在同一门槛上，假阳性率高于单一判据。**两配置嵌套（`N33 ⊂ N89`）
   故 `Δ` 高度相关，严重性低于两独立检验，但**不因此放宽门槛**；缓解手段即第 1 条的强制并报。
3. **禁止「失败后再挑」。**若 A、B 皆不落 `RAISES`，**不得**从 §5 的逐族 `d_auc_sum`
   或任何子集中挑出表现好的部分宣布成功。§5 的逐族诊断是**无门槛并报项，
   在本协议内不构成任何「通过」**，它只为**下一个协议**提供依据；
   任何基于它的动作必须先在新协议中预注册该族，然后重测。

**门槛 +0.02 的依据（定于任何本协议数字之前）**：`joint_tb` 五种子 sd = 0.0011，
故 0.02 ≈ **18 sd**，远超噪声；且表示代价的可用余量为 0.092（0.9084 → 1.0），
0.02 占其 **22%** —— 既有意义又非不可达。

**并报的第三个量（无门槛、不参与判定）**：`Δ(N89 − N33) = Δceiling_B − Δceiling_A`。
