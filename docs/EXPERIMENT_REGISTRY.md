...
| C3-CONFIRM-UNSW16 | 2026-09-05→06 | `9323f73` | `docs/PROTOCOL_C3_CONFIRMATORY_20260905.md`（预登记；§8/§9 两次修正均在读保留集前，理由与证据已记入协议） | 42 与 43 | `features_day_16-09-23.csv`（训练）、`…16-09-30.csv`（选参）、`results/unsw_reserve_16day/`（保留 16 天，**本次首次且唯一一次读取**，762845 窗）、`dataset/unsw/pcap/`（臂 A） | `results/c3_confirmatory_20260905/` | **通过**。C3a 饱和 0.9976（≥0.99）、k1→k10 +0.1986（≥0.15）、单调；C3b′ 两种子各过闸 1 对（Belkin/R2），Δ=+0.0080/+0.0078，区上符号翻转 0/1；C3c Δ≥−0.002 成立。种子间差 0.0001。臂 B：base 0.8620→平滑 0.9183（+0.0563）→定点修 0.9263（+0.0080），逐天 16/16 全为正，与训练天间隔 1–17 天无衰减。**须并报**：第二段作用的那一对区上 +0.0417 而端到端仅 +0.0080（10 类 macro 稀释 5 倍）；第一段:第二段 = 7:1。臂 A 实际证据跨度中位 130s（标称 100s）、p90 308s —— 论文须报实际跨度而非 k×10s | 是（确证测试） |
...
- ~~C3 确证测试的前置：保留 16 天 lenhist 抽取~~ **已完成，测试已跑，见上表**
