...
def cmean1(v,g,k):
    """一维概率的因果滑动平均 —— 与 cmeanM 同 k、同分组、同因果性。

    【审计修正 7】第二段的输出此前【没有】享受第一段的平滑：基模型侧已平滑、
    专用侧是逐窗原始概率，两侧方差尺度不同就等权平均。
    已烧天实测（`blend_smooth.py`，2026-09-06）：补上同口径平滑后
      区上   w=1.0 在三天上全是峰值（0.7835/0.7804/0.7876，在位者 0.5097/0.6294/0.6322）
      端到端 第二段增益 +0.0137/+0.0107/+0.0096 → +0.0511/+0.0286/+0.0298（约三倍）
    即：融合并非必需，`w` 在正确形态下消失。
    """
    if k<=1: return v
    o=np.empty_like(v)
...
def MK(n,seed):
    """【审计修正 8】原实现 RF 同时带 class_weight="balanced" 与 fit_w 的 sample_weight，
    而 sklearn 中两者【相乘】⇒ RF 少数类权重被平方，lr 只加权一次。
    两个候选模型口径不一致，"选 rf 还是 lr"因此有偏。
    改为【只在 fit_w 里加权一次】，两者同口径。
    注意：基模型（RF300）只有 class_weight、不传 sample_weight，本来就是单次加权，不受影响。"""
    if n=="lr": return make_pipeline(StandardScaler(),LogisticRegression(max_iter=2000,C=1.0))
    return RandomForestClassifier(n_estimators=200,random_state=seed,n_jobs=NJ)

_WARNED=set()
def fit_w(m,name,X,y):
    """【审计修正 9】原实现 `except Exception: m.fit(X,y)` 会【静默】退回不加权拟合，
    84 个候选里若有几个悄悄退化，看不出来。改为打印一次警告。"""
    ...
    try:
        ...
        if name not in _WARNED:
            print(f"  ⚠ 加权拟合失败，退回不加权：{name} —— {type(e).__name__}: {e}",flush=True)
            _WARNED.add(name)
        m.fit(X,y)
    return m
...
def arm_b(devs, seed):
    ...
    # 【审计修正 1（2026-09-06）】多对施加时区会重叠，原实现按 dict 顺序边写边量：
    #   后写覆盖先写、无冲突消解，且先施加那一对报的数在被覆盖【之前】量。
    #   CIC 实测：三对压在 {GlobeLamp, GosundPlug, GosundSocket} 上，
    #   两个小区被大区吞掉 99.8%/99.6%，5825 个窗被写 ≥2 次；
    #   6 种施加顺序端到端极差 0.0009 = 第二段增益(0.0033)的 27%。
    # 修正两条：
    #   (a) 施加顺序【按区大小从大到小】—— 确定、可复现，且语义合理：
    #       小区是模型更笃定的判决区，应当覆盖大区；
    #   (b) 逐对记账在【全部写完后的最终 pred】上统一重算，不再边写边量。
    plans=[]
    for (i,j),(rn,mn,nm,_a) in cfgs.items():
        ...
        plans.append((int(sel.sum()),i,j,rn,sel,q))
    plans.sort(key=lambda t:-t[0])                      # 大区先写，小区后覆盖
    pred=o1.copy()
    for n,i,j,rn,sel,q in plans:
        if n==0: continue
        mix=blend(Po_s,q,i,j)
        pred[sel]=np.where(mix[sel]>=0.5,j,i)
    detail=[]
    for n,i,j,rn,sel,q in plans:                        # 写完之后统一记账
        if n==0:
            detail.append((le.classes_[i],le.classes_[j],0,np.nan,np.nan,0,rn)); continue
        n3=int((sel&~np.isin(yo,[i,j])).sum())
        inc_o=float((o1[sel]==yo[sel]).mean())
        acc_o=float((pred[sel]==yo[sel]).mean())        # 同一集合，且是最终 pred
        detail.append((le.classes_[i],le.classes_[j],n,inc_o,acc_o,n3,rn))
