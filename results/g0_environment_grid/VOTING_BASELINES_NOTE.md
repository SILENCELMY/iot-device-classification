...
`w = 1`（`T = 1`）**精确还原**未校准概率（实测 `max|Δ| = 3.6e-12`），
故 §7.6 在 `T = 1` 处退化为 §7.5；`T > 1` 平滑（治过自信），`T < 1` 锐化。
...
| `best_base_posthoc` | `reference` | §7.2 事后上界 | 按 `macro_f1_5class` 选最佳基模型，并列取模型名字典序最靠前；所选模型记于 `selected_base_model` 列 |
