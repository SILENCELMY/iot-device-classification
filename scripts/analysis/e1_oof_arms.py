...
# 臂定义：(oof_mode, cv)。B 的 cv=5 只是上限，GroupKFold 实际折数 = min(cv, 轮次数)。
ARMS = {
    "A": ("random", 5),
    "A_prime": ("random", 2),
    "B": ("grouped", 5),
...
def run_task(features: pd.DataFrame, task: dict, seed: int, args, labels: list[str]) -> list[dict]:
    ...
    for arm, (oof_mode, cv) in ARMS.items():
        model = build_stacking(seed, args.n_jobs, len(labels), oof_mode, cv)
        ...
        rows.append({
            ...
            "oof_mode": oof_mode, "cv_requested": cv, "folds_effective": k,
...
def main() -> int:
    ...
    df.to_csv(OUT_DIR / "e1_arms_raw.csv", index=False, encoding="utf-8-sig")
    ...
    piv = piv.reset_index()
    piv.to_csv(OUT_DIR / "e1_decomposition.csv", index=False, encoding="utf-8-sig")
    ...
    for _, r in piv.iterrows():
        ...
        print(f"    A′ gain = {r['A_prime']:+.6f}   (随机 K=2)")
        print(f"    B  gain = {r['B']:+.6f}   (轮次分组)")
    ...
    with (OUT_DIR / "e1_arms.json").open("w", encoding="utf-8") as f:
    ...
    print(f"\n输出：{OUT_DIR.relative_to(REPO_ROOT)}/"
          "{e1_arms_raw.csv, e1_decomposition.csv, e1_arms.json}")
    return 0
