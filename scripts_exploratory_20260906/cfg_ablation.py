...
SEEDS=[42,43]          # 消融只需两个 seed（本实验方差极小）
CKPT="/home/lmy/cic_probe/_ckpt_abl"
            ...
            for (i,j) in candA:
                ...
                for mn,_k in MASKS:
                    for nm in CANDS:
                        ...
                        if acc>bestA[2]: bestA=(mn,nm,acc,inc)
                if bestA[0] is None or bestA[3] is None: continue
                if bestA[2] > bestA[3]+MARGIN:
                    cfg3[(i,j)]=(bestA[0],bestA[1],bestA[2])
                    print(f"        [仅A|S|=1] {L5[i]}|{L5[j]:9s} {bestA[3]:.4f}→{bestA[2]:.4f}"
                          f"  {bestA[1]}/掩{bestA[0]}",flush=True)
            print(f"      第二段仅A：过闸 {len(cfg3)} 对",flush=True)
            ...
            for uname, src, tgt in OUTER:
                ...
                if gsel.any():
                    ...
                    for (i,j),(mn,nm,a) in cfg3.items():
                        ...
                        pm=fit_w(MK(nm),nm,Xs[ms][:,keep],(ys5[ms]==j).astype(int))
                        q=pm.predict_proba(Xt[gsel][:,keep])[:,1]
                        pred_g3[sel]=np.where(q[sel]>=0.5,j,i)
                    pred_h3=pred_h.copy(); pred_h3[gsel]=pred_g3
                    ...
                    pred_h0=pred_h.copy(); pred_h2=pred_h.copy(); pred_h3=pred_h.copy()
                ...
                f_h3=f1_score(yt5,pred_h3,average="macro",labels=np.arange(len(L5)))
                ...
                print(f"    {uname} seed{seed}  flat={f_flat:.4f}  分层={f_h0:.4f}"
                      ...
                      f"仅A={f_h3:.4f}   Δ(S2)={f_h-f_h0:+.4f}  Δ(A+B)={f_h2-f_h0:+.4f}  "
                      f"Δ(仅A)={f_h3-f_h0:+.4f}",flush=True)
                ...
                rows.append({"unit":uname,"seed":seed,"flat":f_flat,"hier":f_h0,
                             ...
                             "gw_flat":g_flat,"gw_hier":g_h0,"gw_cfg":g_h,"gw_cfg2":g_h2,"cfg3":f_h3,
                             "n_gate":len(cfg),"n_gate2":len(cfg2)})
        R=pd.DataFrame(rows); R.to_csv("/home/lmy/cic_probe/cfg_ablation.csv",index=False)
        print("\n=== 汇总（5 类窗口级 macro-F1）===",flush=True)
        print(R.groupby("unit")[["flat","hier","hier_cfg","hier_cfg2","cfg3"]].mean().round(4).to_string(),flush=True)
        print("\n=== 消融：双inner 的优势来自 |S|=1 还是来自第二环境验证 ===",flush=True)
        print(f"  分层（不修）        {R.hier.mean():.4f}",flush=True)
        print(f"  cfg_S  R2+R3→R4     {R.hier_cfg.mean():.4f}   Δ={R.hier_cfg.mean()-R.hier.mean():+.4f}   |S|=2，无验证",flush=True)
        print(f"  cfg_A  R2→R3        {R.cfg3.mean():.4f}   Δ={R.cfg3.mean()-R.hier.mean():+.4f}   |S|=1，无验证",flush=True)
        print(f"  cfg_AB R2→R3 + R3→R2 {R.hier_cfg2.mean():.4f}   Δ={R.hier_cfg2.mean()-R.hier.mean():+.4f}   |S|=1，有验证",flush=True)
