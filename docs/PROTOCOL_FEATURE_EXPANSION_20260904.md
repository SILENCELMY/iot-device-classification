...
**日期**：2026-09-04
**性质**：表示容量扩充 + 诊断。**只测不选** —— 不让流程在新池上做任何删除决策。
...
新增 **33 列 / 3 族**（族名由 `derive_families` 的 `c.split("_")[0]` 机械导出）。
...
| 配置 | 列数 | 说明 |
|---|---:|---|
| `full94` | 94 | **逐字节不变**，直接复用 `results/robust_v2/raw_all/features_raw_all_w10.csv`（md5 `703984b6ad2fde2f45e0cce1c6df31be`） |
| `full94+N` | **127** | `full94` 按 `(label, round, source_file, window_id)` 左连接新 33 列 |
...
主量 = **同环境天花板的变化** `Δceiling = joint_tb(full94+N) − joint_tb(full94)`，
`joint_tb` 沿用 `run_histint.joint_eval`（H = R2+R3+R4，前 4 时间块训练、第 5 块测试），
取 `best_base`（三非 stacking 模型逐种子取 max），五种子均值。
`joint_tb(full94)` = **0.9084**（`r567_94dim_retest_20260904/deltas.csv`，sd 0.0011）。
...
2. **候选清单锁定**：runner 中新列名与 §2 逐字一致，数量恰为 33，以 `assert` 保证。
...
tshark 重解 + 新列计算约 **10 min**；`coord1_diag` 在 127 列上约 **1 min**
（`pair_auc` 单次实测 0.023 s，9 任务 × 10 类对 × 14 族）；
`joint_tb` 4 模型 × 5 种子约 **10 min**。合计约 **21 min/单跑、42 min/双跑**。
