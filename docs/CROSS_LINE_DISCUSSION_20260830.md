...
## 19. 20260901 重点提案：跨轮次失稳的公开数据复现与少量标签 commissioning 闭环（提交 Opus 审阅）

**性质**：项目负责人与 Fable 5 在 D12 / P2-UNSW-TEST1 冻结判定完成后的新方向讨论记录，供 Opus
作 **scope、优先级与是否立项** 的裁决。**本节不是协议，不授权实现、smoke、正式运行或读取新模型结果**；
不修改 `experiment_protocol_final.md`，不重开 D12、CPD、Route A/B、M1--M6 的任何冻结判定，
也不把 oracle ceiling 改称部署收益。若 Opus 批准，必须另写结果不可见状态下的独立冻结协议。

### 19.1 一页结论：为什么这条线值得重点评估

项目建议把下一阶段的唯一候选主线收缩为两个问题：

1. **现象问题**：同一物理设备在同一时间轮次内可以高精度识别，但换到未见时间轮次后是否发生
   系统性识别下降；这一现象能否从受控自采数据复现到 UNSW 自然流量数据？
2. **方法问题**：在保留历史数据/历史模型的前提下，能否用目标轮开始阶段的**少量可靠标签**完成
   commissioning，提高目标轮后续未标注流量的 macro-F1 与最差设备 F1，并胜过所有同标签预算基线？

**【判断】这条线有较高研究与应用价值，且成功可能性显著高于继续搜索无标签代理量。**理由不是
“降低难度”本身，而是已有证据已经把信息边界定位清楚：M2/M3 表明类别条件块级失配具有较大的
oracle 可恢复性；Route A/B、M4、M5-T、M6 与 D12 又共同表明该结构不能从当前无标签可观测量中稳定
估计。少量入网确认不是把 oracle 上界偷换成部署收益，而是为原本不可识别的问题加入**最小必要监督**，
并让“历史知识是否能以更低标签成本适配未来流量”成为可证伪的方法问题。
...
同轮次高精度不等于跨轮稳定
        ↓
自采受控数据 + UNSW 自然数据的跨轮现象验证
        ↓
无目标标签下 observable estimability 不足（既有冻结负结果）
        ↓
历史模型 + 少量目标轮前段确认标签
        ↓
目标轮后段未来流量的稳定识别恢复
...
第三个更难的问题——“自采训练后直接识别 UNSW 的同类但未见物理设备”——同时跨设备实例、数据集、
行为方式和链路层，暂列 Future Work / 高风险扩展，**不作为本轮方法或论文成立的生死门**。

### 19.2 与最初主线的关系：有设定转向，但不是换题

最初冻结主线研究“环境变化 → 混淆结构变化 / Stacking 失配 → 无标签预警与规避”。新提案保留了
根问题、数据和外层评价对象，但改变了部署信息条件与方法目标：

| 维度 | 最初主线 | 本提案 |
|---|---|---|
| 根问题 | 环境变化使 IoT 分类器与集成关系失稳 | **时间轮次变化使同一设备识别失稳** |
| 自采任务 | Single / Joint / LORO / Position / Jitter | 先聚焦 **Single / Joint / LORO** |
| 外部数据 | UNSW 上检验 CPD / Stacking 机制 | UNSW 上匹配三轮任务，检验**识别下降本身** |
| 原方法出口 | 无标签风险代理、模型选择或门控 | **历史知识锚定的少量标签适配** |
| 部署承诺 | 目标环境零标签 | 入网/新阶段一次性少量确认，之后自治运行 |
| 主结果 | selection regret / Stacking gain | 标签效率曲线、未来块 macro-F1、最差设备 F1 |

因此必须如实表述：**这是从“无标签规避”转向“低标签恢复”的新设定**，不能拿来救回原 RQ3/RQ4；
但它仍然回答项目最初的实际问题——历史环境训练出的识别器如何面对未来环境。既有负结果不是废料，
而是设定转向的证据：

```text
oracle recoverability ≠ observable estimability ≠ deployability
```

新协议若成立，补上的正是最后一步：在显式、有限、可审计的目标标签预算下，把 recoverability 转成
deployable commissioning。CPD 继续只作历史描述量，不恢复为机制、代理或部署指标。

### 19.3 UNSW 是否适合：适合“任务设计匹配”，不声称“采集机制相同”

**【事实】项目使用的公共数据集身份**为 UNSW IoT Traffic Traces / UNSW--Sivanathan IoT Traces：
2016-09-23 至 2016-10-12 共 20 天网关侧 Ethernet pcap，23 个 IoT MAC，许可 MIT-0。一天一个 pcap
混合多台设备并不造成标签歧义：`eth.src` 命中设备 MAC 记该设备上行，`eth.dst` 命中记该设备下行，
随后按 `(device, 10s window)` 独立汇总；MAC、设备名、日期与窗口 ID 均不进入模型特征。

两套数据的采集机制必须明确分开：

| 维度 | 自采数据 | UNSW |
|---|---|---|
| 抓取 | 每设备/轮次约 1 小时，802.11 + radiotap | 每日约 24 小时，网关侧 Ethernet |
| 行为 | 受控 IoT 行为，例如灯按 10s 开/关 | 自然被动运行，无统一动作脚本 |
| 窗口 | 10s，R2--R4 每类每轮 362--390 行 | 10s；当前提取只保留 `>=2` 包的活跃格 |
| 特征 | 94 维 | 61 维公共链路层特征 |
| “轮次” | 重复采集轮次；同轮不同设备并非同一墙钟时刻 | 从 20 天池中为每设备独立随机抽取的一小时段 |

因此新 UNSW 实验的合法身份是：**匹配一小时时长、10s 窗口、三轮和评价任务的外部复现**；不是
10s 开关行为、802.11 特征或物理采集条件的逐项复制。若效果跨这两种采集机制仍一致，提供的是更强的
外部支持；若不一致，则是自然被动流量下的边界条件。

### 19.4 设备类型覆盖与不能声称的语义匹配

UNSW 的设备清单可以支撑设备身份级跨时段研究，也覆盖项目关心的大部分设备形态：

| 形态 | UNSW 实际覆盖 |
|---|---|
| 摄像头 | 8 个 MAC ID / 7 个物理设备名：Dropcam、Samsung SmartCam、TP-Link Cam、Netatmo Welcome 等 |
| 插座/开关 | TP-Link Smart Plug、Belkin Wemo Switch、iHome，共 3 台 |
| 灯 | LiFX Smart Bulb，1 台 |
| 传感器 | 运动、烟雾、气象、睡眠传感器各 1 台 |
| 门窗接触传感器 | **无明确对应设备**；Belkin Wemo Motion 是运动传感器，不得写成门窗传感器 |

这意味着：UNSW 足以回答“同一摄像头/插座/灯/传感器跨时段是否失稳”，但不能声称严格复刻自采
`Camera / Light_T1 / Light_XM / Sensor / Socket` 的五类语义轴——UNSW 只有一台灯，也没有明确门窗
接触传感器。新主实验以**物理设备身份为类**，设备类型只用于分层描述与逐类案例，不把粗类型映射
强行伪装成同一标签空间。

### 19.5 小时级机械支撑审计（未运行模型，只决定可执行性）

**【事实】2026-09-01 已对现有 20 天派生特征作只读支撑审计**：每日日原点下按
`hour_bin = floor(window_id / 360)` 聚合，统计每台设备有多少个一小时段达到给定的有效 10s 窗口数；
没有训练、评分或读取任何新模型结果。23 台设备中，至少存在 3 个合格小时的数量为：

| 每小时最低有效窗口 | 可提供至少 3 个小时的设备数 |
|---:|---:|
| 1 | 21 / 23 |
| 30 | 18 / 23 |
| 60 | 17 / 23 |
...
极端稀疏设备说明不能强行使用全部 23 台：Blipcare 全 20 天当前口径仅 7 个有效窗口，
InsteonCam_wifi 45 个，NestProtect 82 个。**随机抽时段无法制造不存在的数据。**处理原则应是：

1. 先在任何模型结果不可见时冻结纯支撑纳入门；
2. 达门设备进入主实验，之后才固定随机分组与随机抽时；
3. 稀疏设备作独立在线率/不可执行性描述，不复制窗口、不制造空特征、不在结果后补换设备；
4. 阈值本身尚未冻结。`>=100 窗/小时 → 16 台 → 5/5/6 三个面板` 只是一个支撑充分、规模接近
   自采五类任务的**候选设计**，须由 Opus 在协议中裁决，不能把本节的机械清点当成正式纳入结果。

### 19.6 RQ1 候选实验：严格复现 Single / Joint / LORO 任务矩阵

若 Opus 批准，UNSW 的每台入选设备从 20 天可用小时池中按冻结随机种子、不放回抽取三个合格的一小时段，
允许来自不同日期；不同设备不要求使用相同墙钟时刻。抽取顺序机械映射为自采正常轮次
`R2 / R3 / R4`。设备面板也只经冻结随机种子生成，不按模型效果、设备难度或事后混淆调整。

每个设备面板运行与自采论文相同的 7 个核心任务：

| 任务 | 训练 / 测试 | 角色 |
|---|---|---|
| `single_round_R2` | R2 内分层 70/30 | 同轮次可识别性 |
| `single_round_R3` | R3 内分层 70/30 | 同轮次可识别性 |
| `single_round_R4` | R4 内分层 70/30 | 同轮次可识别性 |
| `joint_R2_R3_R4` | 三轮合并后分层 70/30 | 已见全部轮次时的插值上界 |
| `loro_R2_R3_to_R4` | R2+R3 → R4 | 未见 R4 的跨轮泛化 |
| `loro_R2_R4_to_R3` | R2+R4 → R3 | 未见 R3 的跨轮泛化 |
| `loro_R3_R4_to_R2` | R3+R4 → R2 | 未见 R2 的跨轮泛化 |

同轮次和 Joint 的随机分层 70/30 是为与自采历史主表逐项对齐，必须标为 **session 内上界**；鉴于
§11.3 已量化随机窗口划分的乐观偏差，正式协议还应预声明对应的按每设备小时内前/后段 time-block
敏感性，不能只给随机上界。敏感性不替换匹配主口径，也不得在主口径不利后升格。

RQ1 的核心量不是 CPD，而是：

```text
drop_single = mean(F1_single_R2, F1_single_R3, F1_single_R4) - mean(F1_three_LORO)
drop_joint  = F1_joint_R2_R3_R4 - mean(F1_three_LORO)
```

主指标为设备身份级 macro-F1，同时报告 accuracy、逐设备 F1、最差设备 F1、混淆矩阵与各随机面板结果。
是否构成“外部复现”必须在结果前冻结：至少要包含下降方向、实际量级、面板/设备聚类不确定性及
不能由单个困难设备独占的稳健性门。不得在结果后挑摄像头、换面板、换小时或改成有利的类型级标签。

### 19.7 RQ2 候选方法：历史锚定、少量标定、未来块测试

RQ2 的真实部署序列应固定为：

```text
历史 R2/R3 有标签数据（或对应另两轮）
        ↓
训练并冻结基础模型 / 历史可靠性结构
        ↓
目标轮开始阶段每设备少量入网确认标签
...
只拟合低容量适配器
        ↓
目标轮后续时间块不再读取标签，持续自动识别
```

对每个 LORO 目标轮，目标小时必须先按时间切成 commissioning 前段与完全留出的未来后段；标签预算
只从前段取，后段标签只能用于一次性最终评分，不能进入正则、早停、阈值、模型选择或事后筛设备。
候选预算继承 §11.4 / §13.4：每类 `n in {5,10,20,50,100}`，主交付物是完整标签效率曲线，不能挑一个
有利预算点。

当前证据最强的唯一方法候选是**源锚定的 C 级类别条件元层适配器**：冻结基模型，仅用少量目标前段
标签估计类别阈值与类别×模型块权重，并向源参数收缩。M2/M3 只证明该参数化具有 oracle recoverability
和跨数据集亚型支持，**不保证 few-shot 方法成功**；正式方法仍须允许失败。

同标签预算基线是生死线，至少包括：

1. 无适配 Best Single（默认部署候选）；
2. 原始 Stacking / 当前固定集成；
3. 将相同目标标签加入 RF 的直接更新（合并权重与类别平衡先冻结）；
4. 只用目标前段标签拟合的低容量分类器；
5. 简单概率/阈值校准；
6. 完整 target-CF 只作有标签 ceiling，不参与可部署方法胜负。

可发表的方法胜利不能只是“标签有用”，而必须在预声明低预算区间同时满足：相对 Best Single 有正收益、
胜过全部同预算基线、跨自采与 UNSW / 面板方向稳定、最差设备或最差目标轮不恶化，并报告标签、时间、
计算与存储成本。若直接 RF 更新或目标域小模型获胜，则只得到标签价值曲线，C 级方法主张失败；不得换
适配器、删轮次或扩大预算挽救。

### 19.8 实际应用价值：不是“重新标完整数据”，而是有限 commissioning

本提案对应的部署场景不是运营方持续人工标注，而是设备入网、固件/网络条件变化或长期运行后的一次性
有限确认：管理员、资产清单或受控注册流程为每台设备提供少量可靠窗口，历史模型保留大部分已学知识，
低容量适配器快速更新，之后对新流量自治识别。

它解决的是**已知设备或已知类别在新时间阶段发生漂移**，不解决历史中从未出现的新类别。相对完整
本地重训，潜在价值是更少标签、更低计算、更快恢复与保留历史知识；但这些都必须用同预算基线和成本表
证明，不能由“19 个参数”直接推出。

这一场景也给负结果留下清晰边界：若无需多少目标标签，直接重训就稳定获胜，工程建议就是直接重训；
若低预算 C 级适配稳定胜出，才形成“历史锚定能提高标签效率”的方法贡献。两种结果都不得混写。

### 19.9 对 SCI 二区竞争力的影响（条件式，不作录用承诺）

§11.2 将当前项目评为“二区边缘”，核心短板包括单测试床/少物理设备、没有取胜的方法、UNSW 未承担
足够主实验重量。D12 已证明原 CPD 机制检验不能补上这些缺口。本提案若通过，恰好补三个最关键环节：

1. **外部有效性**：同一 Single/Joint/LORO 任务在受控自采与自然 UNSW 上运行；
2. **正向方法贡献**：从解释失效推进到可部署的低标签恢复；
3. **实际闭环**：历史训练 → 入网确认 → 未来时间块，而非随机混合目标窗口的离线增益。

**【判断】若 RQ1 在多个随机设备面板上显示非平凡跨轮下降，且 RQ2 在自采与 UNSW 上以预冻结协议
稳定战胜同标签预算强基线，论文会从“解释性/负结果为主的二区边缘稿”推进为具有完整
问题—外部复现—方法—部署验证链条的二区候选。**RQ3 跨数据集同类设备识别不是必要条件。

反之，下列情况不能靠叙事补救：UNSW LORO 与 Single/Joint 无实际差距；方法收益只有零点几千分之一；
收益由单设备/单面板支撑；同预算 RF 更新同样或更好；随机划分有提升而时间后段测试无提升。出现任一项，
须按冻结分支降级，不能宣称“更接近二区”。期刊分区与录用仍受方法新颖性、已发表基线、统计稳健性、
写作和期刊 scope 影响，本节不作概率或录用保证。

### 19.10 建议的唯一执行顺序与停止规则

若 Opus 批准 scope，建议严格串行，避免再次分叉：

1. **只做数据构造协议**：冻结设备支撑门、随机分组、随机种子、小时候选、三轮映射、每轮定额、
   重复随机划分数量与排除规则；生成 manifest 后停止，不看模型结果。
2. **RQ1 一次性揭示**：只运行 Single / Joint / LORO 与预声明 time-block 敏感性；按冻结门裁定
   `TEMPORAL_FAILURE_EXTERNALLY_SUPPORTED` 或边界/失败分支。
3. **只有达到预声明可恢复空间才进入 RQ2**：若 UNSW 无足够 LORO headroom，不在 UNSW 上事后制造
   方法收益；自采方法是否仍单独继续由协议预先说明。
4. **RQ2 先冻结后运行**：唯一方法族、预算曲线、同预算基线、未来块、主指标、最差轮门、停止规则
   全部写死；正式结果只揭示一次。
5. **RQ3 不启动**：跨数据集同类设备识别留 Future Work，除非前两问完成后由 Opus 另立 scope。

全程继续禁止网络、代理和安装依赖；不修改既有冻结结果；机械重复与长跑监控可交 Luna Max，研究问题、
纳入门、方法族、通过线、例外和最终裁定不得下放。

### 19.11 请求 Opus 明确裁决的七项

请 Opus 不评价预期分数，而是先对以下 scope / 协议身份作书面裁决：

1. 是否批准把下一阶段唯一候选主线定为 **RQ1 跨轮外部复现 + RQ2 少量标签恢复**；
2. 是否同意它是原主线的低标签新设定，原无标签 RQ3/RQ4 与全部冻结负结果原样保留；
3. 是否批准 UNSW 以设备身份为标签、每设备独立随机三小时、映射 R2/R3/R4、运行 7 个匹配任务；
4. 设备纳入门、每小时窗口定额、面板规模、随机划分重复数和 RQ1 通过线应取何值；
5. RQ2 是否只允许“源锚定 C 级适配器”一个方法族，以及低预算主张区间和同预算基线清单；
6. EC-MDM / M2/M3 是否只作为方法参数化动机与 oracle 证据进入正文，而非无标签机制或部署收益；
7. 是否同意 RQ3 跨数据集同类设备识别不作为本轮必做项，待前两问完成后另行决策。

在这七项得到明确回复并形成独立冻结协议前，本节只是一份高价值候选提案，**不得据此启动任何真实
UNSW 重采样模型、few-shot 适配器、调参或试跑。**
